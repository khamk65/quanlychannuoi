...
{
    public partial class ad_manage_giamgiu : Form
    {
        private Database database;

        public ad_manage_giamgiu()
        {
            InitializeComponent();
        ...
        private void ShowkhugiamgiuData()
        {
            try
            {
                // Lấy dữ liệu từ bảng khugiamgiu
                DataTable khugiamgiuData = database.GetData("khugiamgiu");

                // Hiển thị dữ liệu trên DataGridView
                GridViewAccounts.DataSource = khugiamgiuData;
            }
            catch (Exception e)
            {
                MessageBox.Show("Error showing khugiamgiu data: " + e.Message);
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            if (GridViewAccounts.SelectedRows.Count > 0)
            {
                ...
                bool deleteSuccess = database.DeleteData("khugiamgiu", Convert.ToInt32(primaryKeyValue));

                if (deleteSuccess)
                {
                    // Refresh DataGridView để hiển thị dữ liệu mới
                    ShowkhugiamgiuData(); // Gọi lại hàm tải dữ liệu
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            bool insertSuccess = database.InsertData("khugiamgiu", new Dictionary<string, object>
        {
            { "ten", ten },
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            if (GridViewAccounts.SelectedRows.Count > 0)
            {
                ...
                bool updateSuccess = database.UpdateData("khugiamgiu", Convert.ToInt32(primaryKeyValue), new Dictionary<string, object>
            {
                { "ten", ten },
                { "nguoilienhe", nguoilienhe },
                { "email", email },
